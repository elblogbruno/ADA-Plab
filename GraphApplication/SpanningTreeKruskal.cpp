...
CSpanningTree SpanningTreeKruskalOLD(CGraph& g)
{
	struct comparator {
	...
	CSpanningTree tree(&g);

	for (CVertex &v : g.m_Vertices) v.m_KruskalLabel = 0;
	int label = 1;
	for (CVertex &v : g.m_Vertices) {
		v.m_KruskalLabel = label++;
		for (CEdge *pE : v.m_Edges)	if (pE->m_pDestination->m_KruskalLabel==0) queue.push(pE);
	}
	while (!queue.empty()) {
		CEdge* pE = queue.top();
...
CSpanningTree SpanningTreeKruskal(CGraph& g)
{
	...
	for (CEdge& e : g.m_Edges) queue.push(&e);

	int label = 1;
	for (CVertex& v : g.m_Vertices) v.m_KruskalLabel = label++;
